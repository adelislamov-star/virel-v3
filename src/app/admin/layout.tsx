...
import {
    ...
    LineChart, Activity, UserCheck2, MapPin, PieChart,
    BarChart3, TrendingUp, RefreshCcw, Search as SearchIcon, FileText,
    DollarSign, Crown, ConciergeBell,
    FolderOpen, Tag, Sliders, Wrench, ListOrdered, Layers, BookOpen,
...
const sections = [
  {
        ...
        items: [
          ...
          { href: '/admin/fraud',              label: 'Fraud Monitor',      icon: Shield,        badge: 'fraud' },
  ...
  },
  {
        ...
        items: [
          ...
          { href: '/admin/analytics/model-performance',  label: 'Model Performance',icon: Activity,   badge: null },
          ...
          { href: '/admin/analytics/owner',              label: 'Owner Analytics',  icon: PieChart,   badge: null },
              ],
  },
  {
        label: 'Performance',
        items: [
          { href: '/admin/analytics/unit-economics', label: 'Unit Economics', icon: BarChart3,   badge: null },
          { href: '/admin/lost-revenue',             label: 'Lost Revenue',   icon: TrendingUp,  badge: null },
          { href: '/admin/retention',                label: 'Retention',      icon: RefreshCcw,  badge: null },
          { href: '/admin/seo',                      label: 'SEO Health',     icon: SearchIcon,  badge: null },
  ...
  },
  {
        ...
        items: [
          ...
          { href: '/admin/attributes',        label: 'Attributes',      icon: Sliders,      badge: null },
          { href: '/admin/masters/services',  label: 'Services',        icon: Wrench,       badge: null },
          { href: '/admin/masters/call-rates',label: 'Call Rates',      icon: ListOrdered,  badge: null },
          { href: '/admin/masters/locations', label: 'Locations',       icon: Layers,       badge: null },
  ...
  },
  {
        label: 'System',
        items: [
          { href: '/admin/sla',             label: 'SLA Monitor',  icon: Timer,     badge: null },
          ...
          { href: '/admin/audit-log',       label: 'Audit Log',    icon: SearchIcon,badge: null },
                ...
                <aside className="w-60 flex-shrink-0 flex flex-col border-r border-zinc-800/50" style={{ background: 'var(--admin-bg)' }}>
                  {/* Logo */}
                        <div className="px-5 pt-6 pb-4">
                                  <p className="text-sm font-semibold tracking-widest text-zinc-400 uppercase">Virel</p>p>
                                  <p className="text-[11px] text-zinc-600 mt-0.5">Operations Platform</p>p>
                        </div>div>
                          ...
                          {section.items.map(item => {
                                          ...
                                          return (
                                                              <Link
                                                                                  ...
                                                                                  <span className="flex-1 min-w-0 truncate">{item.label}</span>span>
                        ...
                        <div className="px-4 py-3 border-t border-zinc-800/50">
                                  <div className="flex items-center justify-between">
                                              <div className="min-w-0">
                                                            <p className="text-xs font-medium text-zinc-300 truncate">{user?.name ?? '...'}</p>p>
                                                            <p className="text-[11px] text-zinc-600 truncate">{user?.email ?? ''}</p>p>
                                                {user?.roles?.[0] && (
                            <p className="text-[10px] text-amber-500/70 uppercase tracking-wider mt-0.5">{user.roles[0]}</p>p>
                                                            )}
                                              </div>div>
                                              <button
